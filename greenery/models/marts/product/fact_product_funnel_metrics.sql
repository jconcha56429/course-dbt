...
((SELECT add_to_cart_count::numeric FROM add_cart_funnel_count)/(SELECT COUNT(session_id)::numeric FROM dbt.dbt_jacob_c.fact_session_funnel)*100) AS add_to_cart_funnel_rate,
((SELECT checkout_count::numeric FROM checkout_funnel_count)/(SELECT add_to_cart_count::numeric FROM add_cart_funnel_count)*100) AS checkout_funnel_rate
